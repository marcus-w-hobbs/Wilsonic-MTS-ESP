...
**2026-08-18 — SUBSET-MEL-000: the 72 embedded subsets of an eikosany are 36 by algebra, none of the dekanies is proper or CS, and propriety inverts between the eikosany and its subsets.** (EXECUTED; exact-rational construction and CS; frozen melodic v0.1.0 + triad v1.1.0; `results/subsetmel000.jsonl` (360 rows), `results/subsetmel000_summary.json`, `results/scl/subsetmel000/` (21 files); runner `subsetmel000.py`.) For each of five seed eikosanies (classic, the CS-EIK flagship {1,7,9,11,15,29}, and the top-3 CS winners by margin) the census enumerates 6+6+30+15+15 = 72 index-structural subsets and records their Johnson-graph shared-tone matrix (seed-independent for collision-free seeds). H-SM1 held as derived: fix-IN(x) and fix-OUT(x) dekanies are exact inversion pairs (x·CPS(5,2) vs CPS(5,3) of the other five) — identical on every melodic measure, (P,S) swapped, 30/30 pairs; hexany (x in, y out) and (y in, x out) are transpositions, 75/75; two-in/two-out tetrads are harmonic/subharmonic inversions, 75/75 — so a seed has 6 dekany, 15 hexany and 15 tetrad classes. H-SM2 held exactly (classic 26/30 hexanies CS with precisely the two composite-9 classes {1,3,5,9}, {1,3,7,9} failing; every CS-winner seed 30/30). H-SM3 was refuted for the flagship in the informative direction: **0/60 dekanies across all seeds are proper and 0/60 are CS** (the 1-3-5-7-9 dekany has 12 exact CS-violation classes, matching Wilson–Grady's need for a 14-tone completion), and hexany propriety runs the wrong way — the improper classic has 10/30 non-improper hexanies (four strictly proper classes) while the first strictly proper eikosany has 6/30 and {1,3,11,13,25,27} has 0/30. Harmonically the CS-winner dekanies are thin (exact P+S mean 7.5–10.8 vs the classic's 27.5; the classic's richest subset is the improper, non-CS 1-3-5-7-9 dekany at 41), 60/60 dekanies have P ≠ S, and the anchored scorer finds 10/75 harmonic tetrads with no exact triad at all (modal (P,S) = (1,0)) — "tetrads are trivially all-triads-present" is false under this scorer. Rails: tempered ≥ exact at 2¢ on all 360 rows; at 3¢, 68 rows fall below exact, all 68 explained by the frozen scorer's per-triple ε-guard (raw ≥ exact throughout).

**2026-08-18 — SUBSET-MEL-000: the frozen melodic axis is nearly blind at the dekany level, and where it sees, it disagrees with the harmonic axis.** (EXECUTED; same receipts; the fingerprint lens is POST-HOC.) Twenty-two of the thirty dekany classes — every class of the four CS-winner seeds except two — carry one identical M1/M2/M3 profile (6 gap classes, 2.371 bits, improper with 8 violating span pairs, exactly 12 exact CS-violation classes), reproduced by coincidence-free controls CPS(5,2){31,37,41,43,47} and {17,19,23,29,31}: at this level the frozen scores are functions of the seed pitch-arrangement, and only exact coincidences (the composite 9 in {1,3,5,7,9,11}) move them. Consequently the melodic-first ordering has one distinct key on two seeds' dekanies and two on two others; where it discriminates, Spearman melodic-vs-harmonic is 0.37 (classic dekanies), 0.31 (classic hexanies), −0.28 pooled over 60 dekanies (CS-first vs harmonic −0.49) — H-SM5's "< 0.5" held everywhere except the flagship (0.65, carried by a single pair). Input to the SUBSET-MEL-001 brainstorm (G-013): score 6 dekany classes not 12; do not expect propriety or CS to descend from a good eikosany; the aggregator needs graded melodic quantities (violation sizes, near-CS distance, evenness), not class counts; and the spice must be listed by name — here the 1-3-5-7-9 dekany.
