...
**2026-08-18 — BRIDGE-001b: the BRIDGE-001 refutation is robust to the tuning objective and to the completion rule; the k₂ sweep is 98% junk and exposes that count-based survival is not a survival measure; orwell-22 retuned to the eight EG4 images is the tempered-faithful corner.** (EXECUTED; `results/bridge001b.jsonl` + `bridge001b_uncontained.jsonl.gz` + `bridge001b_summary.json`; 33,711 (mapping, N, val) rows from sweeping the second comma over the whole kernel box for all 1,615 monotone (comma, val) pairs, two tunings per row, frozen melodic v0.1.0 on every contained host window; rail = argmin/prime reproduces BRIDGE-001's front bit-for-bit.) Three pre-registered filters, three answers. (1) **k₂ sweep (H-B3):** magic materializes exactly where predicted (N = 19/22, contained, 9.017¢ → P = 1, dominated) and garibaldi exactly where predicted (N = 7/12/17, uncontained — its EG4 chain span is 24; it had P = 4 at 2¢ all along and needs N ≥ 29); of 1,880 contained rows only 16 are inside the 15¢ ε_bridge budget, and the in-budget prime front is BRIDGE-001's four rows unchanged, all 225/224. The letter of the pre-registered (uncapped) front is falsified by an artifact worth recording: temperaments with 64–155¢ tone errors reach hexany "P = 5–7" at ε = 2 because a grossly detuned image is a *different scale* whose accidental near-arithmetic triples out-count the hexany's eight faces — so a post-hoc triad-identity lens (do the hexany's OWN twelve labelled triads keep their labels?) is added; it agrees with the count lens on every row below ~13¢ and zeroes the artifacts. Method rule for BRIDGE-002: any front over a comma sweep must be error-capped or identity-scored. (2) **Tone-set minimax (H-B4):** every hand-derived pin held — miracle 6.858 → 5.817¢ (not below 4¢: 3/2 and 105/64 pull opposite ways, 21/16 caps the gain), orwell 2.727 → 2.571¢, and the named error order flips to orwell < meantone 4.82 < magic 5.36 < mothra 5.48 < miracle 5.82. Survival, being translation-invariant, moves independently of absolute error (H-B6): orwell's e3 crosses under 2¢ and its identity survival rises 2 → 4 faces at ε = 2 (all 6 at 4¢), miracle's 5/3 leaves purity and it falls 3 → 2 (full recovery 3 → 5¢). **The tone-set in-budget front is orwell-22 alone** — 271.4246¢ generator, 2.570¢ max tone error, 0 collisions, 4/6 hexany faces at 2¢, 22-note host 2-step, strictly proper, constant structure. **No row under either tuning revives H-B2**: the 2¢ full-survival bridge does not exist at N ≤ 22 for any of 19,818 rank-2 temperaments and either tuning; the one-cent gap is structural. (3) **Two-gap objective (H-B5):** exactly two of BRIDGE-001's four front rows are true 2-step MOS (orwell-22: 42.5/72.0¢, strictly proper; blackjack: 34.1/82.5¢, L/s 2.42, improper but CS); miracle-19/20 are 3-step non-CS (20 is proper, 19 is not); no 3-step contained candidate beats a 2-step one harmonically in budget; the gap-class column is tuning-robust on every front row, so two-gap-ness and survival can be optimized independently — the SPEC's filler-set design (BRIDGE-001c) has a clean objective to target. Unpredicted newcomer worth an ear: doublewide-22 (50/49 ∩ 875/864, period 600¢, 10.2¢, 3 faces, 2-step strictly proper CS).
